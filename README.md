...
`SUPABASE_SERVICE_ROLE_KEY`, `DATABASE_URL`, AI 키와 Telegram 값은 브라우저에 노출하면 안 됩니다. 기본 분석 provider는 외부 호출이 없는 `mock`입니다. 자동 승인은 기본값이 꺼져 있으며 `--dry-run`으로 정책 결과를 먼저 확인합니다.
