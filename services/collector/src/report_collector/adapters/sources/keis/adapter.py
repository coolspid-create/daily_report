...
DETAIL_PATTERN = re.compile(r"categoryIdx=(\d+)&pubIdx=(\d+)")
DATE_PATTERN = re.compile(r"\d{4}[.-]\d{2}[.-]\d{2}")


class KeisResearchAdapter(SourceAdapter):
    """공개 목록의 onclick 인자를 공식 상세 URL로 변환합니다."""
    ...
    def _parse_list(self, html: str) -> list[DiscoveredItem]:
        ...
        items = [item for node in soup.select("tr.flex-md-order") if (item := _item(node, self.config))]
        if not items:
            raise SourceParseError("KEIS publication list structure changed")
        return items
...
def _item(node: Tag, config: SourceConfig) -> DiscoveredItem | None:
    link = node.select_one(".cell-subject a[onclick*='goDetail']")
    match = DETAIL_PATTERN.search(str(link.get("onclick", ""))) if link else None
    title = link.get_text(" ", strip=True) if link else ""
    if not match or not title or not title_allowed(title, config.filters):
        return None
    category, item_id = match.groups()
    detail_url = str(config.list_url).split("?", 1)[0].replace("/list.do", "/detail.do")
    detail_url = f"{detail_url}?categoryIdx={category}&pubIdx={item_id}"
    return DiscoveredItem(source_item_key=item_id, title=title, detail_url=HttpUrl(detail_url), published_at=_date(node.get_text(" ", strip=True)))
...
def _attachments(soup: BeautifulSoup, base_url: str) -> list[Attachment]:
    links = soup.select("a[href*='/cmmn/download.do?']")
    return [Attachment(url=HttpUrl(urljoin(base_url, str(link.get("href")))), file_name=link.get_text(" ", strip=True) or "한국고용정보원_연구성과.pdf", declared_type="application/pdf") for link in links]


def _summary(soup: BeautifulSoup) -> str | None:
    node = soup.select_one("meta[name='description']") or soup.select_one("#tab-content2")
