...
DOWNLOAD_PATTERN = re.compile(r"fn_downFile\(['\"]([^'\"]+)['\"]\s*,\s*['\"](\d+)['\"]\)")


class KbResearchAdapter(SourceAdapter):
    """KB경영연구소의 공개 보고서와 공식 다운로드 URL을 연결합니다."""
    ...
    def _parse_list(self, html: str) -> list[DiscoveredItem]:
        ...
        items = [item for node in soup.select(".listWrap li") if (item := _item(node, self.config))]
        if not items:
            raise SourceParseError("KB research list structure changed")
        return items
    ...
    async def fetch_detail(self, item: DiscoveredItem) -> SourceDocument:
        ...
        return SourceDocument(
            ...
            attachments=_attachments(html), official_summary=_summary(soup), rights_status=self.config.rights_default,
        )
...
def _item(node: Tag, config: SourceConfig) -> DiscoveredItem | None:
    link = node.select_one("a[href*='reportView.do?reportId=']")
    title_node = node.select_one("h3")
    href = str(link.get("href", "")) if link else ""
    item_id = href.split("reportId=")[-1].split("&")[0] if "reportId=" in href else ""
...
def _attachments(html: str) -> list[Attachment]:
    return [Attachment(url=HttpUrl(f"https://www.kbfg.com/kbresearch/cmm/fms/FileDown.do?atchFileId={match.group(1)}&fileSn={match.group(2)}"), file_name="KB경영연구소_리포트.pdf", declared_type="application/pdf") for match in DOWNLOAD_PATTERN.finditer(html)]


def _summary(soup: BeautifulSoup) -> str | None:
    node = soup.select_one(".viewCont")
