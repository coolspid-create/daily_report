...
export function AutomationStatusPanel({ status }: { status: AutomationStatus | null }) {
  ...
  return (
    <section className="automation-status" aria-label="최근 자동 발행 상태">
      ...
      <dl>
        <div><dt>신규 수집</dt><dd>{status.collectedCount}</dd></div>
        <div><dt>검수 대상</dt><dd>{status.reviewCandidateCount}</dd></div>
        <div><dt>자동 승인</dt><dd>{status.autoApprovedCount}</dd></div>
        <div><dt>검수 대기</dt><dd>{status.exceptionCount}</dd></div>
        <div><dt>실행 당시 피드</dt><dd>{status.publishedCount}</dd></div>
      </dl>
