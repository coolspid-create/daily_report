...
export function ReportActions({ file }: ReportActionsProps) {
  ...
  return (
    <div className="report-actions">
      {primary ? (
        <a className="action action-primary" href={primary.url} download={primary.download || undefined} aria-label={`${primary.label} 파일 다운로드`} {...externalProps}>
          {primary.label} <span className="sr-only">(새 창)</span>
        </a>
      ) : null}
      {hasSeparateSource && (
        <a className="action action-secondary" href={file.sourceUrl} aria-label="원문 ↗ 공식 원문 열기" {...externalProps}>
