...
연속 실패 시 출처는 `DEGRADED`를 거쳐 `DISABLED`가 됩니다. `DISABLED` 출처는 일반 수집 대상에서 제외하고, 보도자료 출처만 마지막 실패 후 24시간마다 한 번 복구 점검합니다. 공식 사이트가 시스템 점검 안내를 반환하면 `SOURCE_MAINTENANCE`로 기록하며 실패 횟수는 증가시키지 않습니다. 공개 화면은 기존 current snapshot을 계속 사용하므로 수집 장애가 즉시 빈 피드가 되지 않습니다.
